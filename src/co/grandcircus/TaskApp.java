...
public class TaskApp {
	...
	public static void main(String[] args) {
		...
		taskList.add(new Task("Alex", "Finish Week 2 Capstone", LocalDate.parse("05/06/2019", Validator.f)));
		taskList.add(new Task("Leo", "Paint Mona Lisa", LocalDate.parse("06/17/1506", Validator.f)));
	...
	public static void listTasks() {
		...
		for (int i = 0; i < taskList.size(); i++) {
			System.out.printf("%-4s%-14s%-34s%-12s%s\n", (i + 1), taskList.get(i).getName(),
					taskList.get(i).getDescription(), taskList.get(i).getDate().format(Validator.f).toString(),
	...
	public static void addTask(Scanner scan) {
		String name = Validator.getLine(scan, "Team Member Name: ");
		String description = Validator.getLine(scan, "Task Description: ");
		description = String.format("%1.33s", description);
		LocalDate date = Validator.getDate(scan, "Due Date: ");
	...
	public static void deleteTask(Scanner scan) {
		listTasks();
		int index = Validator.getInt(scan, "Which task would you like to delete? (0 to cancel): ", 0, taskList.size());
		if (index == 0) {
			System.out.println("\n***Delete Cancelled***");
		} else {
			printHeader();
			...
			System.out.printf("%-4s%-14s%-34s%-12s%s\n", (index + 1), taskList.get(index).getName(),
					taskList.get(index).getDescription(), taskList.get(index).getDate().format(Validator.f).toString(),
	...
	public static void completeTask(Scanner scan) {
		listTasks();
		int index = Validator.getInt(scan, "Which task would you like to complete? (0 to cancel): ", 0,
				taskList.size());
		if (index == 0) {
			System.out.println("\n***Complete Cancelled***");
		} else {
	...
	public static void editTask(Scanner scan) {
		listTasks();
		int index = Validator.getInt(scan, "Which task would you like to edit? (0 to cancel/ -1 to batch complete): ",
				-1, taskList.size());
		if (index == 0) {
			System.out.println("\n***Edit Cancelled***");
		} else if (index == -1) {
			int comp = Validator.getInt(scan, "Enter 0 to batch incomplete, 1 to batch complete: ", 0, 1);
			if (comp == 0) {
				ArrayList<Integer> tracker = Validator.getBatch(scan,
			...
			index = index - 1;
			while (true) {
				String field = Validator.getString(scan, "What would you like to edit?: ");
				if (field.equalsIgnoreCase("name") | field.charAt(0) == 'n' | field.charAt(0) == 'N') {
					String newName = Validator.getLine(scan, "Team Member Name: ");
					taskList.get(index).setName(newName);
					System.out.println("\n***Edit Recorded***");
					break;
				} else if (field.equalsIgnoreCase("description") | field.charAt(0) == 'd') {
					String newDesc = Validator.getLine(scan, "Description: ");
					...
					LocalDate newDate = Validator.getDate(scan, "Date: ");
					taskList.get(index).setDate(newDate);

					System.out.println("\n***Edit Recorded***");
					break;
				} else if (field.equalsIgnoreCase("status") | field.charAt(0) == 's' | field.charAt(0) == 'S') {
					int newStatus = Validator.getInt(scan, "Enter 0 for Incomplete, 1 for Complete: ", 0, 1);
					if (newStatus == 1) {
						taskList.get(index).setComplete();
					}
					if (newStatus == 0) {
